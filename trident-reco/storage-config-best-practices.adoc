...
Trident使用主機群組來存取其所配置的磁碟區（LUN）。根據預設、Trident會使用名為「Trident」的主機群組、除非您在組態中指定不同的主機群組名稱。Trident本身不會建立或管理主機群組。您必須先建立主機群組、才能在Trident上設定E系列儲存後端。確保主機群組中的所有Kubernetes工作節點iSCSI IQN名稱均已更新。
...
為了確保只有獲授權的節點組能夠存取透過Cloud Volumes Service 效益管理系統配置的Volume、請在建立Cloud Volumes Service 一個效益管理系統的同時、為匯出原則設定適當的規則。透過Trident在Cloud Volume Services上配置磁碟區時、請務必使用後端檔案中的「exportRule」參數、以便存取所需的Kubernetes節點。
...
我可以使用後端組態檔中的「limitVolume Sizes」參數、限制Trident在Cloud Volume Services for AWS上建立的磁碟區大小上限。如果要求的磁碟區大小高於設定值、設定此參數可確保資源配置失敗。
